...
This tutorial is part 2 of a series. The xref:tutorials/nbm-selection-1.adoc[previous tutorial] covered the basics of component-wide selection handling in NetBeans - how to provide objects from a ` xref:wiki:netbeansdevelopperfaq:DevFaqWindowsTopComponent.adoc[TopComponent]`'s ` xref:wiki:netbeansdevelopperfaq:DevFaqLookup.adoc[Lookup]`, and how to write other components that are sensitive to the `Lookup` of whatever component has focus. This tutorial assumes that you have completed the previous tutorial, and have the source code to hand, since we will be modifying it in this part.
